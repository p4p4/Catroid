...
public class IfThenLogicActionTest extends AndroidTestCase {

	private static final int IF_TRUE_VALUE = 42;
	private static final int IF_FALSE_VALUE = 32;
	private static final int IF_INIT_VALUE = 69;
	private static final String TEST_USERVARIABLE = "testUservariable";
	private Sprite testSprite;
	private StartScript testScript;
	private IfThenLogicBeginBrick ifLogicBeginBrick;
	private IfThenLogicEndBrick ifLogicEndBrick;
	private Project project;
	private IfThenLogicBeginBrick ifLogicBeginBrick2;
	private IfThenLogicEndBrick ifLogicEndBrick2;
	...
	public void testNestedIfBrick() throws InterruptedException {
		...
		ifLogicBeginBrick = new IfThenLogicBeginBrick(validFormula);
		ifLogicEndBrick = new IfThenLogicEndBrick(ifLogicBeginBrick);
		repeatBrick.setLoopEndBrick(new LoopEndBrick(repeatBrick));

		ifLogicBeginBrick2 = new IfThenLogicBeginBrick(validFormula);
		ifLogicEndBrick2 = new IfThenLogicEndBrick(ifLogicBeginBrick2);
	...
	public void testIfNotBrick() throws InterruptedException {
		SetVariableBrick setVariableBrickInit = new SetVariableBrick(new Formula(IF_INIT_VALUE), userVariable);
		SetVariableBrick setVariableBrickIfTrue = new SetVariableBrick(new Formula(IF_TRUE_VALUE), userVariable);
		...
		testScript.addBrick(setVariableBrickInit);
		testScript.addBrick(ifLogicBeginBrick);
		...
		assertEquals("IfBrick not executed as expected", Double.valueOf(IF_INIT_VALUE), userVariable.getValue());
	}
	...
	public void testNullFormula() {
		...
		Action ifLogicAction = ExtendedActions.ifThenLogic(testSprite, null, ifAction);
	...
	private void testFormula(Formula formula, Object expected) {
		...
		testScript = new StartScript();
		ifLogicBeginBrick = new IfThenLogicBeginBrick(formula);
		ifLogicEndBrick = new IfThenLogicEndBrick(ifLogicBeginBrick);
		testScript.addBrick(ifLogicBeginBrick);
		testScript.addBrick(setVariableBrickIfTrue);
		testScript.addBrick(ifLogicEndBrick);
